...
function meetocean_ensure_schema(PDO $pdo): void
{
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS meetocean_rooms (
            ...
            KEY idx_meetocean_rooms_activity (last_activity_at),
    ...
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS meetocean_participants (
            ...
            updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            UNIQUE KEY uniq_meetocean_participant_client (room_id, client_id),
            KEY idx_meetocean_participants_room_updated (room_id, updated_at),
            CONSTRAINT fk_meetocean_participants_room FOREIGN KEY (room_id) REFERENCES meetocean_rooms(id) ON DELETE CASCADE,
    ...
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS meetocean_signals (
            ...
            created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
            KEY idx_meetocean_signals_recipient (room_id, recipient_client_id, id),
            KEY idx_meetocean_signals_created (created_at),
    ...
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS meetocean_transcripts (
            ...
            created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
            KEY idx_meetocean_transcripts_room_id (room_id, id),
            CONSTRAINT fk_meetocean_transcripts_room FOREIGN KEY (room_id) REFERENCES meetocean_rooms(id) ON DELETE CASCADE,
...
function meetocean_require_user(PDO $pdo): array
{
    ...
    if (!in_array(MEETOCEAN_MODULE_ID, $visibleModules, true)) {
        oceanos_json_response([
            'ok' => false,
            'error' => 'forbidden',
            'message' => 'MeetOcean n est pas active pour ce compte.',
        ], 403);
    }
...
function meetocean_public_user(array $user): array
{
    return [
        'id' => (int) ($user['id'] ?? 0),
...
function meetocean_languages(): array
{
    return [
        'fr-FR' => 'Francais',
        'en-US' => 'Anglais',
...
function meetocean_normalize_language(mixed $language, string $fallback = 'fr-FR'): string
{
...
function meetocean_language_label(string $language): string
{
    $languages = meetocean_languages();
    return $languages[meetocean_normalize_language($language)] ?? $language;
}

function meetocean_clean_client_id(mixed $clientId): string
{
...
function meetocean_clean_room_code(mixed $code): string
{
...
function meetocean_room_title(mixed $title): string
{
    $title = trim(preg_replace('/\s+/', ' ', (string) $title) ?? '');
    if ($title === '') {
        $title = 'Reunion MeetOcean';
    }
...
function meetocean_find_room(PDO $pdo, mixed $room): array
{
    if (is_numeric($room)) {
...
function meetocean_media_state(array $input): array
{
...
function meetocean_touch_participant(PDO $pdo, array $room, array $user, array $input): array
{
...
function meetocean_json_decode(?string $json): array
{
    $decoded = json_decode((string) $json, true);
    return is_array($decoded) ? $decoded : [];
}

function meetocean_json_encode(array $payload): string
{
...
function meetocean_add_signal(PDO $pdo, array $room, array $input): array
{
    $sender = meetocean_clean_client_id($input['senderClientId'] ?? $input['clientId'] ?? '');
    $recipient = meetocean_clean_client_id($input['recipientClientId'] ?? '');
    ...
    $statement->execute([
        ...
        'signal_type' => $type,
        'payload_json' => meetocean_json_encode($payload),
    ]);
...
function meetocean_signals_since(PDO $pdo, int $roomId, string $clientId, int $sinceId): array
{
    $statement = $pdo->prepare(
        'SELECT id, sender_client_id, recipient_client_id, signal_type, payload_json, created_at
         FROM meetocean_signals
         WHERE room_id = :room_id
           AND id > :since_id
    ...
    foreach ($statement->fetchAll() ?: [] as $row) {
        $signals[] = [
            ...
            'payload' => meetocean_json_decode($row['payload_json'] ?? '{}'),
...
function meetocean_add_transcript(PDO $pdo, array $room, array $user, array $input): array
{
    $clientId = meetocean_clean_client_id($input['clientId'] ?? '');
...
function meetocean_translate_text(PDO $pdo, int $userId, string $text, string $sourceLanguage, string $targetLanguage): string
{
    ...
    $sourceLabel = meetocean_language_label($sourceLanguage);
    $targetLabel = meetocean_language_label($targetLanguage);
...
function meetocean_ensure_translation(PDO $pdo, int $userId, array $row, string $targetLanguage): array
{
    $sourceLanguage = meetocean_normalize_language($row['source_language'] ?? 'fr-FR');
    $targetLanguage = meetocean_normalize_language($targetLanguage, $sourceLanguage);
    $text = trim((string) ($row['text'] ?? ''));
    $translations = meetocean_json_decode($row['translations_json'] ?? '{}');
    ...
    try {
        $translated = meetocean_translate_text($pdo, $userId, $text, $sourceLanguage, $targetLanguage);
    ...
    if ($translated !== '') {
        ...
        $row['translations_json'] = meetocean_json_encode($translations);
...
function meetocean_transcript_payload(array $row, string $targetLanguage): array
{
    $sourceLanguage = meetocean_normalize_language($row['source_language'] ?? 'fr-FR');
    $targetLanguage = meetocean_normalize_language($targetLanguage, $sourceLanguage);
...
function meetocean_transcripts_since(PDO $pdo, int $roomId, int $sinceId, string $targetLanguage, int $userId): array
{
    $statement = $pdo->prepare(
        'SELECT id, room_id, user_id, client_id, speaker_name, source_language, text, translations_json, is_final, created_at
         FROM meetocean_transcripts
         WHERE room_id = :room_id
           AND id > :since_id
         ORDER BY id ASC
         LIMIT 100'
    ...
    $items = [];
    foreach ($statement->fetchAll() ?: [] as $row) {
        $row = meetocean_ensure_translation($pdo, $userId, $row, $targetLanguage);
        $items[] = meetocean_transcript_payload($row, $targetLanguage);
    }
...
function meetocean_dashboard(PDO $pdo, array $user): array
{
    ...
    return [
        'ok' => true,
        'managedBy' => 'OceanOS',
        'currentUser' => meetocean_public_user($user),
        'canManage' => meetocean_is_admin($user),
        'languages' => meetocean_languages(),
        'defaults' => [
