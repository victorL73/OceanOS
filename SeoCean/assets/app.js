...
async function requestJson(url, options = {}) {
  ...
  if (!response.ok || payload.ok === false) {
    throw new Error(payload.message || payload.error || "Erreur SeoCean.");
  }
  return payload;
}
...
function renderAdvisor(payload) {
  ...
  elements.advisorBody.textContent = summary.body || "SeoCean traduit les chiffres en actions simples.";
...
async function boot() {
  ...
  try {
    ...
    setVisible(true);
    setMessage(error.message || "SeoCean indisponible.", "error");
  }
}
